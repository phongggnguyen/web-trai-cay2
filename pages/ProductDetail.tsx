...
interface ProductDetailProps {
  ...
  onProductClick?: (product: Product) => void; // Optional prop for related products
}

const ProductDetail: React.FC<ProductDetailProps> = ({ product, onAddToCart, onNavigate, onProductClick }) => {
  ...
  return (
    <div className="w-full max-w-[1440px] mx-auto px-4 md:px-8 py-6 md:py-10">
      ...
      <div className="flex flex-wrap gap-2 mb-8 items-center text-sm">
        <button onClick={() => onNavigate(Page.HOME)} className="text-text-muted dark:text-gray-400 font-medium hover:underline">Trang chủ</button>
        <span className="text-text-muted dark:text-gray-600">/</span>
        <button onClick={() => onNavigate(Page.PRODUCT_LIST)} className="text-text-muted dark:text-gray-400 font-medium hover:underline">{product.category || 'Trái cây nhập khẩu'}</button>
        <span className="text-text-muted dark:text-gray-600">/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 xl:gap-14">
        ...
        <div className="lg:col-span-7 flex flex-col gap-4">
          ...
          <div className="flex gap-4 overflow-x-auto hide-scrollbar py-1">
             ...
             <button className="relative flex-shrink-0 w-24 h-24 rounded-xl overflow-hidden border-2 border-transparent hover:border-primary/50 cursor-pointer transition-colors bg-gray-100 dark:bg-surface-dark flex items-center justify-center">
        ...
        <div className="py-8 text-base text-gray-600 dark:text-gray-300 leading-relaxed max-w-4xl space-y-4 animate-fade-in-up">
          {activeTab === 'description' && (
            <>
              ...
              <div className="my-6 bg-background-light dark:bg-surface-dark p-6 rounded-2xl border border-border-color dark:border-border-dark flex gap-4 items-center shadow-sm">
                <div className="size-12 rounded-full bg-yellow-100 dark:bg-yellow-900/30 flex items-center justify-center flex-shrink-0 text-yellow-600 dark:text-yellow-400">
                  <span className="material-symbols-outlined text-2xl">lightbulb</span>
                </div>
                <div>
                  <h4 className="font-bold text-text-main dark:text-white">Mẹo nhỏ từ Tiệm:</h4>
                  <p className="text-sm">Để trái cây luôn giòn, hãy bảo quản trong ngăn mát tủ lạnh và chỉ rửa trước khi ăn. Ăn lạnh sẽ ngon hơn gấp đôi!</p>
                </div>
              </div>
            </>
          )}
      ...
      <div className="mt-12 md:mt-20">
        <h2 className="text-2xl md:text-3xl font-black text-text-main dark:text-white mb-8">Đánh giá từ khách hàng</h2>
        <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 md:p-10 shadow-sm border border-border-color dark:border-border-dark">
          <div className="flex flex-wrap gap-x-12 gap-y-8">
            {/* Overall Score */}
            <div className="flex flex-col gap-2 min-w-[150px]">
              <p className="text-text-main dark:text-white text-6xl font-black leading-none tracking-tighter">{product.rating}</p>
              <div className="flex gap-1 text-primary">
                {[1, 2, 3, 4, 5].map((s) => (
                  <span key={s} className="material-symbols-outlined fill-current">star</span>
                ))}
              </div>
              <p className="text-gray-500 text-base font-bold">{product.reviews} đánh giá</p>
            </div>
            
            {/* Progress Bars */}
            <div className="grid min-w-[280px] flex-1 grid-cols-[20px_1fr_40px] items-center gap-y-3">
              {[5, 4, 3, 2, 1].map((star) => (
                <React.Fragment key={star}>
                  <p className="text-sm font-bold dark:text-white">{star}</p>
                  <div className="flex h-2.5 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                    <div className="rounded-full bg-primary" style={{ width: star === 5 ? '80%' : star === 4 ? '15%' : '5%' }}></div>
                  </div>
                  <p className="text-sm text-gray-500 text-right font-medium">{star === 5 ? '80%' : star === 4 ? '15%' : '5%'}</p>
                </React.Fragment>
              ))}
            </div>

            {/* Write Review Action */}
            <div className="flex flex-col justify-center gap-3 w-full md:w-auto md:ml-auto border-t md:border-t-0 md:border-l border-border-color dark:border-border-dark pt-6 md:pt-0 md:pl-10">
              <p className="font-bold text-center md:text-left dark:text-white">Đã mua sản phẩm này?</p>
              <button className="px-8 py-3 rounded-full border-2 border-text-main dark:border-white dark:text-white font-bold hover:bg-text-main hover:text-white dark:hover:bg-white dark:hover:text-text-main transition-all">
                Viết đánh giá
              </button>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-8">
          {[
            { name: 'Lan Hương', date: '2 ngày trước', content: 'Cherry siêu ngon, giòn rụm luôn ạ. Shop giao hàng nhanh thần tốc, đặt lúc 9h mà 10h đã có để ăn rồi.', color: 'bg-blue-100 text-blue-600', letter: 'L' },
            { name: 'Minh Tuấn', date: '5 ngày trước', content: 'Hàng chất lượng, đóng gói kỹ. Tuy nhiên shipper hơi khó tính xíu. Nhưng vì cherry ngon nên bỏ qua hết.', color: 'bg-pink-100 text-pink-600', letter: 'M', img: true }
          ].map((review, i) => (
            <div key={i} className="bg-surface-light dark:bg-surface-dark p-6 rounded-2xl border border-border-color dark:border-border-dark">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`size-10 rounded-full ${review.color} flex items-center justify-center font-bold`}>{review.letter}</div>
                  <div>
                    <h4 className="font-bold text-sm dark:text-white">{review.name}</h4>
                    <p className="text-xs text-green-600 flex items-center gap-1 font-bold"><span className="material-symbols-outlined text-[14px]">check_circle</span> Đã mua hàng</p>
                  </div>
                </div>
              ...
              {review.img && (
                 <div className="mt-3">
                    <img src={product.image} className="size-16 rounded-lg object-cover" alt="Review" />
                 </div>
              )}
            </div>
          ))}
      ...
      <div className="mt-20 mb-10">
        <div className="flex justify-between items-end mb-8">
          ...
          <button onClick={() => onNavigate(Page.PRODUCT_LIST)} className="text-primary font-bold hover:underline">Xem tất cả</button>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
          {relatedProducts.map(p => (
            <div key={p.id} className="group flex flex-col gap-3 cursor-pointer" onClick={() => handleRelatedClick(p)}>
