    ...
    <div className="w-full max-w-[1280px] mx-auto px-4 sm:px-6 py-8 md:py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">
            Bạn có <span className="text-primary font-bold">{items.length} sản phẩm</span> trong giỏ hàng
          </p>
        </div>
        <button 
          onClick={() => onNavigate(Page.PRODUCT_LIST)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {/* Note/Empty area */}
          <div className="p-6 rounded-xl border border-dashed border-border-color dark:border-border-dark bg-surface-light/50 dark:bg-surface-dark/50 text-center">
            <p className="text-sm text-gray-500 dark:text-gray-400">Bạn muốn mua thêm trái cây khác? <button onClick={() => onNavigate(Page.PRODUCT_LIST)} className="text-primary font-bold hover:underline">Dạo cửa hàng ngay</button></p>
          </div>
        </div>
        ...
        <div className="lg:col-span-4">
          <div className="sticky top-24 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col items-center justify-center p-4 bg-surface-light dark:bg-surface-dark rounded-xl text-center gap-2 border border-border-color dark:border-border-dark">
                <span className="material-symbols-outlined text-primary text-3xl">local_shipping</span>
                <span className="text-xs font-bold text-text-main dark:text-white">Giao hàng siêu tốc<br/>trong 2h</span>
              </div>
              <div className="flex flex-col items-center justify-center p-4 bg-surface-light dark:bg-surface-dark rounded-xl text-center gap-2 border border-border-color dark:border-border-dark">
                <span className="material-symbols-outlined text-primary text-3xl">verified_user</span>
                <span className="text-xs font-bold text-text-main dark:text-white">Đổi trả miễn phí<br/>nếu hư hỏng</span>
              </div>
            </div>
          </div>
        </div>
